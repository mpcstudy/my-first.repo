...
//include getenv("DOCUMENT_ROOT")."/config/config_Info.php";
...
function checkForm(){
	...
	if (f.checkID.value == '' || f.checkID.value == null)
	{
		//alert("이메일 주소를 입력하세요.");
		...
		if (f.checkID.value.search(reg) == -1) {
...
function checkFormNormalID(){
    ...
    if (f.checkID.value.length < 4 || f.checkID.value.length > 15){ 
        //alert("아이디는 4~15자 이내여야 합니다."); 
        alert(" Must be 4~15 characters."); 
        f.checkID.value="";         
        f.checkID.focus();
        return false; 
    } 
...
-->
		...
		<TR>
			<TD height="15" align="center" bgcolor="#DDDDDD"><b><font color=black> ID CHECK </font></b> </TD>
		</TR>
