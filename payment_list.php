...
if($key_word) {
	...
	$add_srchquery .= " AND  ((C_FIRSTNAME LIKE '%$key_wordStr%') OR (C_LASTNAME LIKE '%$key_wordStr%'))";
}
...
//total record
$query = "SELECT COUNT(UID) FROM tbl_order 
					WHERE UID <> '' " . $add_allquery . $add_srchquery . $add_query;
...
while($list=mysqli_fetch_array($result, MYSQLI_ASSOC)) {
	...
	if ($ORDSTATUS == '1') $ORDSTATUSstr = "Paid";
	else $ORDSTATUSstr = "Failed";

	
	if ($_SESSION['ss_ALEVEL'] == 1) {
		$OPTIONstr = "<a href=# onclick=\"javascript:goDelete('$adm_id');\" $setTag><img src='/images/sbtn-delete.png' border=0 align='absmiddle'></a>";
	...
	$strList .= "
		...
		<tr onclick=\"viewwin=dhtmlwindow.open('viewwinpopup', 'iframe', '/admin/doc/payment_info.php?mode=view&UID=$UID', 'Payment Information', 'width=650px,height=440px,left=300px, top=150px, center=0, resize=0,scrolling=1'); return false\" onmouseover=this.style.background='#EFF1FC' onmouseout=this.style.background='' style='cursor:pointer;cursor:hand;' >
			<td height='25' class='tc'>$list_number</td>
			<td class='t2'>$USERID</td>
		  <td class='t2'><b>$C_FIRSTNAMEstr, $C_LASTNAMEstr</b></A></td>
		  <td class='t2'>$ORDNMSstr</td>
		  <td class='t2'>$ORDPAYDATEstr</td>
		  <td class='t1'>$ $ORDTOTAL</td>
		  <td class='tc'>$ORDSTATUSstr</td>
		  <!--td class='tc'>$OPTIONstr</td-->
		</tr>
		<tr>
			<td height='1' colspan='7' style='border-bottom-width:1px; border-bottom-color:#cccccc; border-bottom-style:dotted;'>
			</td>
		</tr>
...
if( $total_count < 1 ) {
	...
	$strList = "
		<tr height=25><td colspan='7' align=center><B>No Data</B></td></tr>
		<tr>
			<td height='1' colspan='7' style='border-bottom-width:1px; border-bottom-color:#cccccc; border-bottom-style:dotted;'>
			</td>
		</tr>		
...
function goDelete(adm_id) {
	if(confirm("Are you sure to delete?")) {
		document.form2.adm_id.value=adm_id;
...
<input type=hidden name=modeStr value="USER">
...
<p><img src="images/icon-list.png" align="absmiddle"><font class="title">PAYMENT</font></p>

<FORM METHOD=POST NAME=form1 ONSUBMIT='return goSearch(this)' action='<?=$_SERVER["PHP_SELF"]?>'>
<INPUT TYPE=HIDDEN NAME=view VALUE='payment_list'>
...
<table width=95% align="center" border="0" cellspacing="0" cellpadding="5">
  <tr>
    ...
    <td width=60%><!--div align=right><a href="/admin/index.php?view=payment_info&mode=create&page=<?=$page?>&key_word=<?=$key_word?>&column=<?=$column?>&switched=<?=$getSWHStr?>&sorting_type=<?=$sorting_type?>&switch=<?=$switch?>" ><span class='update' style='cursor:pointer'>CREATE</span></a></div--></td>
  </tr>
</table>
...
<td class="tdthl"><a href=/admin/index.php?view=payment_info&switched=<?=$getSWHStr?>&page=<?=$page?>&key_word=<?=$key_word?>&column=<?=$column?>&sorting_type=<?=$sorting_type?>&switch=user_userid>User ID</a></td>
<td class="tdthl"><a href=/admin/index.php?view=payment_info&switched=<?=$getSWHStr?>&page=<?=$page?>&key_word=<?=$key_word?>&column=<?=$column?>&sorting_type=<?=$sorting_type?>&switch=user_fname>User Name</a></td>
<td class="tdthl"><a href=/admin/index.php?view=payment_info&switched=<?=$getSWHStr?>&page=<?=$page?>&key_word=<?=$key_word?>&column=<?=$column?>&sorting_type=<?=$sorting_type?>&switch=user_phone>Service Name</a></td>
<td class="tdthl"><a href=/admin/index.php?view=payment_info&switched=<?=$getSWHStr?>&page=<?=$page?>&key_word=<?=$key_word?>&column=<?=$column?>&sorting_type=<?=$sorting_type?>&switch=user_grade>Date</a></td>	
<td class="tdthl">Service Fee</td>		
<td class="tdthc"><a href=/admin/index.php?view=payment_info&switched=<?=$getSWHStr?>&page=<?=$page?>&key_word=<?=$key_word?>&column=<?=$column?>&sorting_type=<?=$sorting_type?>&switch=user_status>Status</a></td>	
<!--th><?=$OPTIONTITLEstr?></th-->
